...
import { BarChart3, Building2, Users, TrendingUp, Gauge, Activity } from 'lucide-react'
          ...
          {stats.map((stat, i) => {
            ...
            return (
              <motion.div
                ...
                className="bg-card/30 p-8 text-center relative group hover:bg-card/50 transition-colors"
              >
                {/* Corner accent */}
                {i < 4 && (
                  <div className="absolute top-0 left-0 w-12 h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent group-hover:via-primary/60 transition-all" />
                )}
                <Icon className="w-8 h-8 text-primary/40 mx-auto mb-4 group-hover:text-primary/60 transition-colors" />
                <div className="text-3xl sm:text-4xl font-bold text-foreground mb-1 font-mono tracking-tight">{stat.value}</div>
                <div className="text-sm text-foreground/80 font-medium">{stat.label}</div>
                <div className="text-[10px] data-text text-muted-foreground/40 mt-1 tracking-wider">{stat.sub}</div>
              </motion.div>
            )
          })}
        </div>
